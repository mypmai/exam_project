...
class UserManager(models.Manager):
	...
	def registerVal(self,postData): #validation on postData fields
		...
		if not Name_Regex.match(postData['alias']):
			results['status']=True
			results['errors'].append("Alias can only be letters")	
	...
	def createUser(self,postData): #adding user into database
		...
		self.create(first_name=postData['first_name'],alias=postData['alias'],email=postData['email'],password=password, birthdate=postData['birthdate'])
